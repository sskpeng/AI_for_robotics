...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        for b in range(2):
            ...
            Xi.value[n+b+2][0]        +=  motion[b] / motion_noise
    #
    # extra steps to reduce the matrix size
    sdim = 2 * (1 + num_landmarks)
    cnt = 0
    while Omega.dimx > sdim or cnt > 2 * N:
        cnt += 1
        o_dim = Omega.dimx
        Omega_1 = Omega.take(list(range(2, o_dim)), list(range(2, o_dim)))
        Xi_1 = Xi.take(list(range(2, o_dim)),[0])
        A = Omega.take([0, 1], list(range(2, o_dim)))
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        #
        Omega = Omega_1 - A.transpose() * B.inverse() * A
        Xi = Xi_1 - A.transpose() * B.inverse() * C
        #print('dim O=', Omega_1.dimx)
        #print('dim X=', Xi_1.dimx)

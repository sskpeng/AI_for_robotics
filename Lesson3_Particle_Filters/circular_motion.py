...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        beta = dist / self.length * math.tan(alpha)
        if beta <> 0.0:
            
            R = dist / beta

            cx = self.x - math.sin(self.orientation) * R
            cy = self.y + math.cos(self.orientation) * R

            x_new = (cx + math.sin(self.orientation + beta) * R) 
            y_new = (cy - math.cos(self.orientation + beta) * R) 
            o_new = (self.orientation + beta) % (2*3.14)
        else:
            x_new = self.x + math.cos(self.orientation) * dist
            y_new = self.y + math.sin(self.orientation) * dist
            o_new = self.orientation
        myrobot = robot(length)
        myrobot.set(x_new, y_new, o_new)
        result = myrobot
